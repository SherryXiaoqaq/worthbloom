...
完成接线并断电复查后，把 `include/app_config.h` 的 `#define WB_ENABLE_NEOPIXEL 0` 改成 `1`。Adafruit 官方建议数据线上串 300–500Ω 电阻，并在灯带电源端并联 500–1000µF 电容；5V 灯珠配 3.3V 主控时使用电平转换更可靠：[NeoPixel 官方接线建议](https://learn.adafruit.com/adafruit-neopixel-uberguide/best-practices)。

8 颗灯按最坏 60mA/颗估算可接近 0.48A，不能从普通 GPIO 取电。当前代码还把最大亮度限制在 72/255，但供电仍应留足余量。

## 第四阶段：震动

先买“带驱动的振动马达模块”，不要把裸马达直接接 GPIO。不同模块的电压、驱动方式差别很大，因此购买后先把正反面和型号拍给我，再确定三根线怎么接。
