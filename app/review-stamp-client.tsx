...
export default function ReviewStampClient(){
  ...
  if(done)return <main className={styles.stage}><section className={`${styles.card} ${styles.thanks}`}><div className={styles.seal}>收到</div><span className={styles.brand}>你的视角已经送达</span><h1>谢谢你认真看完，<br/>决定仍然属于她。</h1><p>你不会看到其他人的回答，这张链接也已经自动失效。</p><p className={styles.cta}>{CTA_TEXT}</p>{!claimResult?.claimed&&<button type="button" className={styles.submit} disabled={busy||!claimCredentials} onClick={()=>void tryClaim()}>{busy?'正在认领…':'登录后认领好好值'}</button>}{claimResult?.claimed&&<small className={styles.claimOk}>已认领 {claimResult.points} 好好值</small>}{claimResult&&!claimResult.claimed&&claimResult.reason&&<small className={styles.claimHint}>{claimResult.reason}</small>}<Link className={styles.claimHome} href="/">前往好好花登录或注册</Link></section></main>;
