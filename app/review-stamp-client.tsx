...
const LINK_STATE_COPY:Record<string,string>={USED:'这张链接已经被使用过了',REVOKED:'这张链接已被撤销',REQUEST_DECIDED:'这个心愿已经完成决定',EXPIRED:'这张链接已过期'};
...
export default function ReviewStampClient(){
  ...
  async function submit(event:FormEvent){event.preventDefault();if(!role||!stamp||!picked.length){setError('请先选择你的身份、判断章和至少一个理由。');return}setBusy(true);setError('');try{const response=await fetch('/api/review',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({token,reviewerName:name.trim()||'匿名朋友',reviewerRole:role,stamp,reasons:picked,note})});const data=await response.json() as {reviewId?:string;claimToken?:string;error?:string;code?:string};if(!response.ok)throw new Error(data.error||'提交失败');setDone(true)}catch(reason){setError(reason instanceof Error?reason.message:'提交失败')}finally{setBusy(false)}}

  async function tryClaim(){setBusy(true);try{const response=await fetch('/api/review/claim',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({reviewId:(done&&undefined),claimToken:undefined})});const data=await response.json() as {pointsAwarded?:number;code?:string;error?:string};if(response.ok)setClaimResult({claimed:true,points:data.pointsAwarded??null,reason:null});else if(data.code==='AUTH_REQUIRED')setClaimResult({claimed:false,points:null,reason:'登录后可认领积分'});else setClaimResult({claimed:false,points:null,reason:data.error||'认领失败'})}catch{setClaimResult({claimed:false,points:null,reason:'认领失败'})}finally{setBusy(false)}}

  if(loading)return <main className={styles.stage}><section className={styles.card}>愿望正在展开…</section></main>;
  if((error&&!wish)||linkState!=='ACTIVE')return <main className={styles.stage}><section className={styles.card}><span className={styles.brand}>WORTHBLOOM · 好好花</span><h1>这张邀请卡已经合上</h1><p>{error||LINK_STATE_COPY[linkState]||'链接已失效'}</p><small>每张链接只属于一位朋友，也只能填写一次。</small></section></main>;
  if(done)return <main className={styles.stage}><section className={`${styles.card} ${styles.thanks}`}><div className={styles.seal}>收到</div><span className={styles.brand}>你的视角已经送达</span><h1>谢谢你认真看完，<br/>决定仍然属于她。</h1><p>你不会看到其他人的回答，这张链接也已经自动失效。</p><p className={styles.cta}>{CTA_TEXT}</p>{claimResult?.claimed&&<small className={styles.claimOk}>已认领 {claimResult.points} 积分</small>}{claimResult&&!claimResult.claimed&&claimResult.reason&&<small className={styles.claimHint}>{claimResult.reason}</small>}</section></main>;
