...
export default function WorthBloomMainClient(){
  ...
  return <main className={styles.stage}><div className={styles.phone}>
    ...
    {view==='room'&&active&&<WishRoom key={active.id} data={data} request={active} busy={busy} message={message} onBack={back} onInvite={()=>void invite()} onCopyInvite={copyInvite} onDecide={(choice,noteOverride)=>void decide(choice,noteOverride)} onAddSaving={addSaving} onDecisionNote={setDecisionNote} onEdit={()=>{setEditRequest(active);setCreateOpen(true)}}/>}
