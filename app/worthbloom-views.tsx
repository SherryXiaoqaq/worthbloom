...
export function ProfileView({data,profile,growthAccount,growthEntries,unreadCount,onProfileChange,onAvatarChange,onNavigate,onOpen}:{data:AppData;profile:UserProfile;growthAccount:GrowthAccount;growthEntries:GrowthLedgerEntry[];unreadCount:number;onProfileChange:(profile:UserProfile)=>Promise<void>;onAvatarChange:(avatarDataUrl:string|null)=>Promise<void>;onNavigate:(view:View)=>void;onOpen:(request:PurchaseRequest)=>void}){
  ...
  const displayGrowthEntries=growthEntries.map(entry=>{const request=data.requests.find(item=>item.id===entry.referenceId);const asset=data.assets.find(item=>item.id===entry.referenceId);const title=entry.actionType==='profile_completed'?'完善个人资料':entry.actionType==='decision_with_reason'?`为「${request?.name||'心愿'}」写下决定理由`:entry.actionType==='review_claim'?'提供一份有效朋友回信':entry.actionType==='asset_reflection'?`记录「${asset?.name||'已有物品'}」的真实体验`:entry.actionType;return{id:entry.id,title,date:entry.createdAt,points:entry.delta,limited:entry.limited}});
  ...
  return <section className={`${styles.rootPage} ${styles.profilePage}`}>
    ...
    <section className={styles.profileSection}><div className={styles.profileSectionHeading}><small>常用工具</small><h2>常用功能</h2></div><div className={styles.profileUtilityGrid}><button onClick={()=>onNavigate('device')}><span><Icon name="flower" size={25}/></span><b>电子花</b><small>查看「{deriveDeviceSummary(data).title}」的进度</small></button><button onClick={()=>setPanel('PRIVACY')}><span><Icon name="shield" size={25}/></span><b>隐私与分享</b><small>{profile.shareIdentityDefault==='ANONYMOUS'?'默认匿名':'默认展示昵称'}</small></button></div></section>

    {panel!=='NONE'&&<div className={styles.profileOverlay} role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)setPanel('NONE')}}><section className={styles.profileSheet} role="dialog" aria-modal="true" aria-label={panel==='AVATAR'?'裁剪头像':panel==='EDIT'?'编辑资料':panel==='PRIVACY'?'隐私与分享':panel==='GROWTH'?'好好值规则与明细':'设置'}><header><b>{panel==='AVATAR'?'调整头像':panel==='EDIT'?'编辑资料':panel==='PRIVACY'?'隐私与分享':panel==='GROWTH'?'好好值规则与明细':'设置'}</b><button onClick={()=>setPanel('NONE')} aria-label="关闭"><Icon name="close"/></button></header>
      ...
      {panel==='GROWTH'&&<div className={styles.growthLedger}><section><small>LV.{growth.level} · {growth.name}</small><b>{points}<em> 好好值</em></b><div className={styles.growthProgress}><i style={{width:`${progress}%`}}/></div><p>{growth.next?`再获得 ${growth.next-points} 好好值进入下一级`:'当前已达到最高等级'}</p></section><p>它记录“想清楚—行动—回看”的过程。写下好体验或坏体验得到的分值相同，消费金额也不会改变分数。</p><div className={styles.growthRules}><article><b>+20</b><span>为一个心愿写下决定理由，每个心愿一次</span></article><article><b>+15</b><span>首次回看一件已有物品，后续记录不重复加分</span></article><article><b>+10</b><span>完善昵称和个人简介，每个账户一次</span></article><article><b>+10</b><span>认领一份有效朋友回信，每天最多 3 次</span></article><small>邀请人数、连续登录、消费金额、体验好坏和是否跟随多数意见都不加分。</small></div><div>{displayGrowthEntries.map(entry=><article key={entry.id}><span><b>{entry.title}</b><small>{new Date(entry.date).toLocaleDateString('zh-CN')}{entry.limited?' · 已达当日上限':''}</small></span><strong>{entry.points>0?'+':''}{entry.points}</strong></article>)}</div>{!displayGrowthEntries.length&&<p className={styles.emptyState}>还没有成长记录。</p>}</div>}
    </section></div>}
  </section>;
}
...
export function DeviceView({data,onBack}:{data:AppData;onBack:()=>void}){
  const state=deriveDeviceSummary(data);
  ...
  return <section className={styles.subPage}><PageHeader title="电子花" onBack={onBack}/><aside className={styles.deviceIntro}><span className={styles.deviceIntroIcon}><Icon name="flower" size={22}/></span><div><h2>电子花是什么？</h2><p>它把心愿、存钱目标和已有物资的进度，画成一朵会变化的花，每次陪你关注一件事。</p><small>这是页面里的陪伴面板，不需要购买或连接实体设备。</small></div></aside><article className={styles.devicePanel}><span className={styles.deviceLargeIcon}><Icon name="flower" size={42}/></span><small>此刻陪你关注</small><h2>{state.title}</h2><p>{statusLine}</p><div className={styles.largeProgress}><i style={{width:`${Math.round(state.progress*100)}%`}}/></div><b>{Math.round(state.progress*100)}%</b></article><section className={styles.deviceFacts}><h2>这朵花在看什么</h2><div><span>当前状态</span><b>{mode}</b></div><div><span>{remainingLabel}</span><b>{remainingValue}</b></div><p>进度来自你在花园里的记录。以后如果接入桌面花，设备也可以显示同一份状态；不连接时，页面照常使用。</p></section></section>;
}
