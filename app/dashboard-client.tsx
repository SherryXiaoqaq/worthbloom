...
import { effectiveUsedUnits, remainingUnits } from '@/lib/asset-rules';
...
function TopBar({ title, onBack, action }: { title:string; onBack?:()=>void; action?:React.ReactNode }) {
  return <header className="mobile-topbar"><div className="top-side">{onBack&&<button className="back-button" onClick={onBack} aria-label="返回上一页"><span>←</span>返回</button>}</div><div className="top-title"><small>WORTHBLOOM</small><strong>{title}</strong></div><div className="top-action">{action}</div></header>;
}
...
function CreateAsset({ onBack,onCreated }: { onBack:()=>void; onCreated:()=>void }) {
  const [form,setForm] = useState({ name:'',type:'ITEM' as Asset['type'],purchase_price:'',total_units:'',usage_count:'',current_balance:'',expiry_date:'' });
  const [busy,setBusy] = useState(false);
  const [error,setError] = useState('');
  const update = (key:keyof typeof form,value:string) => setForm(previous=>({...previous,[key]:value}));
  const chooseType=(type:Asset['type'])=>setForm(previous=>({...previous,type,total_units:'',usage_count:'',current_balance:''}));
  const spent=form.type==='STORED_VALUE'&&form.purchase_price&&form.current_balance!==''?Math.max(0,Number(form.purchase_price)-Number(form.current_balance)):null;
  async function submit(event:FormEvent){event.preventDefault();setBusy(true);setError('');try{const payload={...form,purchase_price:Number(form.purchase_price),total_units:form.total_units===''?null:Number(form.total_units),usage_count:form.usage_count===''?0:Number(form.usage_count),current_balance:form.current_balance===''?null:Number(form.current_balance)};const response=await cloudBaseFetch('/api/data',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'add_asset',payload})});await readApi(response);onCreated()}catch(reason){setError(reason instanceof Error?reason.message:'添加失败')}finally{setBusy(false)}}
  return <><TopBar title="添加已有物资" onBack={onBack}/><form className="request-form" onSubmit={submit}><div className="form-intro"><span>ALREADY MINE</span><h1>把已经拥有的，<br/>重新带回生活。</h1><p>以前买过的课程、会员、储值或实物，都可以从这里开始记录。</p></div>
    <label><span>物资名称 *</span><input required value={form.name} onChange={event=>update('name',event.target.value)} placeholder="例如：去年买的瑜伽年卡"/></label>
    <fieldset className="asset-type-picker"><legend>它属于哪一类？ *</legend><div>{(['COURSE','MEMBERSHIP','STORED_VALUE','ITEM'] as Asset['type'][]).map(type=><button type="button" className={form.type===type?'selected':''} onClick={()=>chooseType(type)} key={type}><AssetGlyph type={type}/><span><b>{typeCopy[type]}</b><small>{type==='COURSE'?'按次数逐次使用':type==='MEMBERSHIP'?'不限次，关注到期日':type==='STORED_VALUE'?'每次消费扣减余额':'累计真实使用次数'}</small></span><i>{form.type===type?'✓':''}</i></button>)}</div></fieldset>
    <div className="asset-rule-note"><b>{form.type==='COURSE'?'课程 / 次卡':form.type==='MEMBERSHIP'?'会员 / 年卡':form.type==='STORED_VALUE'?'储值卡 / 余额':'耐用品 / 实物'}</b><p>{form.type==='COURSE'?'购买总次数减去已使用次数，就是现在剩余次数。':form.type==='MEMBERSHIP'?'没有“总次数”，每次记录只增加累计使用次数。':form.type==='STORED_VALUE'?'记录消费时会要求填写金额，并从当前余额中扣除。':'没有“总次数”，记录每一次真正使用即可。'}</p></div>
    <div className="field-pair"><label><span>{form.type==='STORED_VALUE'?'累计储值金额 *':'当时购入金额 *'}</span><div className="money-input"><i>¥</i><input required min="0" step="0.01" type="number" value={form.purchase_price} onChange={event=>update('purchase_price',event.target.value)} placeholder="0"/></div></label><label><span>{form.type==='ITEM'?'保修 / 关注截止日':'有效期'}</span><input type="date" value={form.expiry_date} onChange={event=>update('expiry_date',event.target.value)}/></label></div>
    {form.type==='COURSE'&&<div className="field-pair"><label><span>购买总次数 *</span><input required min="1" step="1" type="number" value={form.total_units} onChange={event=>update('total_units',event.target.value)} placeholder="例如 12"/></label><label><span>已经使用次数</span><input min="0" step="1" type="number" value={form.usage_count} onChange={event=>update('usage_count',event.target.value)} placeholder="例如 3"/></label></div>}
    {form.type==='STORED_VALUE'?<div className="field-pair"><label><span>当前余额 *</span><div className="money-input"><i>¥</i><input required min="0" step="0.01" type="number" value={form.current_balance} onChange={event=>update('current_balance',event.target.value)} placeholder="现在还剩多少"/></div>{spent!=null&&Number.isFinite(spent)&&<small className="field-helper">据此计算，历史已消费 ¥{spent.toLocaleString()}</small>}</label><label><span>历史消费笔数</span><input min="0" step="1" type="number" value={form.usage_count} onChange={event=>update('usage_count',event.target.value)} placeholder="不知道可留空"/></label></div>:form.type!=='COURSE'&&<label><span>历史使用次数</span><input min="0" step="1" type="number" value={form.usage_count} onChange={event=>update('usage_count',event.target.value)} placeholder="不知道可留空，从今天开始记录"/></label>}
    <div className="form-note"><b>同一件物资只保留一套计数规则</b><p>历史次数会直接成为初始使用记录；课程剩余次数和储值余额都会从这个起点继续变化。</p></div>{error&&<p className="form-error">{error}</p>}<button className="main-button" disabled={busy}>{busy?'正在放进物资架…':'添加到我的物资'}</button></form></>;
}
...
function AssetsView({ assets,onUpdated,onAdd }: { assets:Asset[]; onUpdated:()=>void; onAdd:()=>void }) {
  ...
  async function action(actionName:string, assetId:string,amount?:number){setBusy(assetId);setError('');try{const response=await cloudBaseFetch('/api/data',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:actionName,assetId,amount})});await readApi(response);await onUpdated();setConfirmDelete('');setSpendingId('');setSpendAmount('')}catch(reason){setError(reason instanceof Error?reason.message:'操作失败')}finally{setBusy('')}}
  function recordSpend(event:FormEvent,asset:Asset){event.preventDefault();void action('use_asset',asset.id,Number(spendAmount))}
  function toggleSpend(assetId:string){setSpendingId(previous=>previous===assetId?'':assetId);setSpendAmount('')}
  return <><TopBar title="我的物资"/><section className="tab-intro"><span>MY VALUE</span><h1>买下不是结尾，<br/>用起来才是。</h1><p>课程看剩余次数，储值看真实余额，会员和实物记录每一次使用。</p></section>{error&&<p className="form-error list-error">{error}</p>}<section className="asset-list">{assets.map(asset=>{const used=effectiveUsedUnits(asset);const remain=remainingUnits(asset);const usageBase=asset.type==='COURSE'?used:asset.usage_count;const costPerUse=usageBase?Math.round(asset.purchase_price/usageBase):null;const balance=Number(asset.current_balance??0);const expiry=asset.expiry_date?asset.expiry_date.slice(5).replace('-','.'):'长期';const stats=asset.type==='COURSE'?[{value:remain,label:'剩余次数'},{value:costPerUse?`¥${costPerUse}`:'—',label:'当前每次'},{value:expiry,label:'有效期'}]:asset.type==='STORED_VALUE'?[{value:`¥${balance.toLocaleString()}`,label:'当前余额'},{value:`¥${Math.max(0,asset.purchase_price-balance).toLocaleString()}`,label:'累计消费'},{value:expiry,label:'有效期'}]:[{value:asset.usage_count,label:'累计使用'},{value:costPerUse?`¥${costPerUse}`:'—',label:'当前每次'},{value:expiry,label:asset.type==='ITEM'?'关注期限':'有效期'}];const unavailable=asset.type==='COURSE'?remain===0:asset.type==='STORED_VALUE'?balance<=0:false;return <article className={`asset-card asset-card-${asset.type.toLowerCase()}`} key={asset.id}><div className="asset-title"><AssetGlyph type={asset.type}/><div><span>{typeCopy[asset.type]}</span><h2>{asset.name}</h2></div>{asset.type==='STORED_VALUE'&&<em>余额制</em>}</div><div className="asset-stats">{stats.map(stat=><div key={stat.label}><b>{stat.value}</b><span>{stat.label}</span></div>)}</div><div className="asset-actions"><button disabled={busy===asset.id||unavailable} onClick={()=>asset.type==='STORED_VALUE'?toggleSpend(asset.id):void action('use_asset',asset.id)}>{busy===asset.id?'更新中…':unavailable?(asset.type==='COURSE'?'次数已用完':'余额已用完'):asset.type==='STORED_VALUE'?'记录一笔消费':'＋ 记录一次使用'}</button><button className={`stop-track ${confirmDelete===asset.id?'confirming':''}`} disabled={busy===asset.id} onClick={()=>confirmDelete===asset.id?void action('delete_asset',asset.id):setConfirmDelete(asset.id)}>{confirmDelete===asset.id?'确认移除':'停止追踪'}</button></div>{spendingId===asset.id&&<form className="asset-spend-form" onSubmit={event=>recordSpend(event,asset)}><div><label htmlFor={`spend-${asset.id}`}>本次实际消费</label><small>当前余额 ¥{balance.toLocaleString()}</small></div><div className="spend-entry"><span>¥</span><input id={`spend-${asset.id}`} autoFocus required min="0.01" max={balance} step="0.01" type="number" value={spendAmount} onChange={event=>setSpendAmount(event.target.value)} placeholder="0.00"/><button disabled={busy===asset.id}>确认扣款</button></div></form>}{confirmDelete===asset.id&&<small className="delete-hint">再点一次确认移除；这不会影响其他物资。</small>}</article>})}{!assets.length&&<div className="empty-card">还没有物资。把以前买过但仍想好好使用的东西放进来吧。</div>}</section><button className="floating-create" onClick={onAdd}>＋ 添加已有物资</button></>;
}
